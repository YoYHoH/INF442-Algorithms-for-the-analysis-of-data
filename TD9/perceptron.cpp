      ...
      rate(_rate), decay(_decay)
{
    ...
    for(int j = 0; j<_size; j++){
        hidden[j] = new Neuron(_dim, inputs, _activation, _activation_der);
    }

    // output = new Neuron(_size, hidden, sigma, sigma_der);
...
void OneLayerPerceptron::prepareInputs(Dataset *data, int row, int regr, bool print)
{
    ...
    // TODO Exercise 4
    int index = 0;
    std::vector<double> a = data ->getInstance(row);
    for(int i = 0; i<data->getDim(); i++){
        if(i != regr){
            inputs[index] ->setSignal(normalise(a[i], data, i));
            index += 1;
        }
    }
